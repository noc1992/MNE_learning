...
# 4. 데이터 정제 및 리샘플링
  4-1 필터링 배경 
   - 필터는 신호의 일부를 제거하거나 감쇠시킴. 
   - 좁은 주파수 범위로 제한된 아티팩트는 때때로 필터링하여 복구 가능
   - 복구 가능한 아티팩트 느린 드리프트, 전력선 노이즈
    1) 느린 드리프트 
     DC 시프트 보정 비 활성화.
    2) 전력선 노이즈
     MEG - raw 객체에 직접 적용되는 노치 필터로 수행, 감쇠할 주파수 배열 지정
     EEG - 노이즈 영향 없으므로 picks 자력계, 라디오 미터만 필터링
  4-2 리샘플링
   - 신호의 고주파 성분이 중요하지 않고 정확한 타이밍이 필요하지 않은 경우 사용
   - raw.resample(), epochs.resample()및 evoked.resample()
   - 몇가지 함정 : 이벤트 배열의 시간 정밀도에 부정적, 모든 에폭에 아티팩트 발생 가능
